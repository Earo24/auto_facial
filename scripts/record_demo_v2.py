...
def run_demo():
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            # 2. 视频列表 (15秒)
            print("2/6 视频列表")
            page.click('a:has-text("视频列表")', timeout=5000)
            page.wait_for_load_state('networkidle')
            time.sleep(5)
            ...
            print("  - 选择簇查看详情")
            try:
                page.click('text=张海宇', timeout=5000)
            except:
                print("  - 使用备用选择器")
                page.click('[class*="cluster"]', timeout=5000)
            time.sleep(10)

            # 4. 剧集管理 (20秒)
            print("4/6 剧集管理")
            page.click('a:has-text("剧集管理")', timeout=5000)
            page.wait_for_load_state('networkidle')
            time.sleep(5)

            # 5. 视频处理 (15秒)
            print("5/6 视频处理")
            page.click('a:has-text("视频处理")', timeout=5000)
            page.wait_for_load_state('networkidle')
            time.sleep(5)

            # 6. 返回仪表板 (15秒)
            print("6/6 返回仪表板")
            page.click('a:has-text("仪表板")', timeout=5000)
            page.wait_for_load_state('networkidle')
            time.sleep(5)
